...
class MoveItPyPlanner_right(object):
  def __init__(self):
    super(MoveItPyPlanner_right, self).__init__()

    ## BEGIN_SUB_TUTORIAL setup
    ##
    ## First initialize `moveit_commander`_ and a `rospy`_ node:
    moveit_commander.roscpp_initialize(sys.argv)
    # rospy.init_node('moveItPyPlanner_right', anonymous=True)
    ...
    ##get right gripper of the baxter
  ...
  def add_box(self, box_name,frame_id,x,y,z,_size,timeout=4):
    ...
    ## ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ## First, we will create a box in the planning scene at the location of the right finger:
